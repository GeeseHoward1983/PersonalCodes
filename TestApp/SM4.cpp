...
{
    unsigned long MK[4] = { 0 }; //拆分后的4个32bit秘钥
    unsigned long k[36] = { 0 }; //存放MK与系统参数FK异或运算得到的K
    unsigned long i = 0;

    //将密钥拆分为4字
    MK[0] = *((unsigned long*)key);
    MK[1] = *((unsigned long*)key + 1);
    MK[2] = *((unsigned long*)key + 2);
    MK[3] = *((unsigned long*)key + 3);
    ...
    unsigned char output[16])
{
    ...
    while (i < 32)
    {
        ulbuf[i + 4] = sm4F(ulbuf[i], ulbuf[i + 1], ulbuf[i + 2], ulbuf[i + 3], sk[31 - i]);
        i++;
    }
